...
const sign = (req, expires) => {
  ...
  let token = jwt.sign({
    ...
    ,
    (req.otp ? req.otp : '') + pass_sign_token + timeSign + (req.proxy ? req.proxy : (req.origin + req.clientDevice))
    ,
    {
...
class TokenUtil {
  ...
  async requestNewToken(req, res, next) {
    ...
    if (req.username && req.captcha) {
      ...
      req.token = sign(req, '60000'); //tạo token phù hợp với mã OTP đã mã hóa
  ...
  createToken365(req, res, next) {
    ...
    if (req.username) {
      ...
      req.token = sign(req, req.expiresIn ? req.expiresIn : '365d');
