    ...
    */
    ...
    createUsername(req, res, next) {
        ...
        if (!req.error
            ...
            let fullName = req.json_data ? req.json_data.full_name : req.paramS.full_name;
            ...
            if (req.next_status === "NEW-USER") {
                ...
                db.insert(arrObj.convertSqlFromJson("admin_users", jsonAdminUser))
                    ...
                    .catch(err => {
                        res.writeHead(435, { 'Content-Type': 'application/json; charset=utf-8' });
                        res.end(JSON.stringify({ status: 'NOK', message: 'Lỗi tạo user', error: err }));
                    });
            } else {
                //or LOGIN-USER
     ...
     * gồm: res.user_info.username and res.user_info.data
     * @param {*} req 
     * @param {*} res 
     * @param {*} next 
     */
